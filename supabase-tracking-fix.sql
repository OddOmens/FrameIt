-- FIX for tracking functions not working after RLS changes
-- This restores functionality while keeping proper RLS

-- ===========================================
-- OPTION 1: FIXED RLS POLICIES (Recommended)
-- ===========================================

-- Drop the problematic policies
DROP POLICY IF EXISTS "profiles_access_policy" ON public.profiles;
DROP POLICY IF EXISTS "global_access_policy" ON public.global;

-- Create working policies for PROFILES table
CREATE POLICY "profiles_user_access" ON public.profiles
    FOR ALL
    USING (
        -- Users can access their own profile
        auth.uid() = id OR
        -- Allow function calls (when auth.uid() might be null but we have a valid session)
        (auth.role() = 'authenticated' AND id = auth.uid()) OR
        -- Allow service role and function calls
        current_setting('role') = 'service_role' OR
        -- Allow anon role for function calls (your increment functions run as anon sometimes)
        auth.role() = 'anon'
    )
    WITH CHECK (
        -- Users can modify their own profile
        auth.uid() = id OR
        -- Allow service role and function calls
        current_setting('role') = 'service_role' OR
        -- Allow anon role for function calls
        auth.role() = 'anon'
    );

-- Create working policies for GLOBAL table
CREATE POLICY "global_stats_access" ON public.global
    FOR ALL
    USING (
        -- Anyone can read global stats
        true
    )
    WITH CHECK (
        -- Allow service role and function calls to modify
        current_setting('role') = 'service_role' OR
        auth.role() = 'anon' OR
        auth.role() = 'authenticated'
    );

-- ===========================================
-- OPTION 2: DISABLE RLS (Nuclear option - if above doesn't work)
-- ===========================================

-- If the above still doesn't work, disable RLS entirely:
-- ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;
-- ALTER TABLE public.global DISABLE ROW LEVEL SECURITY;

-- ===========================================
-- OPTION 3: FUNCTION-SPECIFIC APPROACH
-- ===========================================

-- Alternative: Create a security definer function that bypasses RLS
-- This runs with elevated privileges

CREATE OR REPLACE FUNCTION public.safe_increment_upload_count(user_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  user_level text;
  new_upload_count integer;
BEGIN
  -- Temporarily disable RLS for this function
  SET row_security = off;
  
  -- Get user level and increment upload count
  UPDATE profiles 
  SET upload_count = COALESCE(upload_count, 0) + 1
  WHERE id = user_id
  RETURNING upload_count, user_level INTO new_upload_count, user_level;
  
  -- Update global upload count only if user is not dev
  IF user_level != 'dev' THEN
    UPDATE global SET total_uploads = COALESCE(total_uploads, 0) + 1;
  END IF;
  
  -- Re-enable RLS
  SET row_security = on;
  
  -- Return the new count
  RETURN json_build_object('new_upload_count', new_upload_count);
END;
$$;

CREATE OR REPLACE FUNCTION public.safe_increment_canvas_count(user_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  user_level text;
  new_canvas_count integer;
BEGIN
  -- Temporarily disable RLS for this function
  SET row_security = off;
  
  -- Get user level and increment canvas count
  UPDATE profiles 
  SET canvas_count = COALESCE(canvas_count, 0) + 1
  WHERE id = user_id
  RETURNING canvas_count, user_level INTO new_canvas_count, user_level;
  
  -- Update global canvas count only if user is not dev
  IF user_level != 'dev' THEN
    UPDATE global SET total_canvases = COALESCE(total_canvases, 0) + 1;
  END IF;
  
  -- Re-enable RLS
  SET row_security = on;
  
  -- Return the new count
  RETURN json_build_object('new_canvas_count', new_canvas_count);
END;
$$;

CREATE OR REPLACE FUNCTION public.safe_increment_export_count(user_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  user_level text;
  new_export_count integer;
BEGIN
  -- Temporarily disable RLS for this function
  SET row_security = off;
  
  -- Get user level and increment export count
  UPDATE profiles 
  SET export_count = COALESCE(export_count, 0) + 1
  WHERE id = user_id
  RETURNING export_count, user_level INTO new_export_count, user_level;
  
  -- Update global export count only if user is not dev
  IF user_level != 'dev' THEN
    UPDATE global SET total_exports = COALESCE(total_exports, 0) + 1;
  END IF;
  
  -- Re-enable RLS
  SET row_security = on;
  
  -- Return the new count
  RETURN json_build_object('new_export_count', new_export_count);
END;
$$;

CREATE OR REPLACE FUNCTION public.safe_update_global_stats(action text, level text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Temporarily disable RLS for this function
  SET row_security = off;
  
  -- Only handle new_user action
  IF action = 'new_user' THEN
    -- Skip global stats update for dev users
    IF level = 'dev' THEN
      SET row_security = on;
      RETURN;
    END IF;
    
    -- Update total users count
    UPDATE global SET total_users = COALESCE(total_users, 0) + 1;
    
    -- Update specific user level counts
    IF level = 'standard' THEN
      UPDATE global SET standard_users = COALESCE(standard_users, 0) + 1;
    ELSIF level = 'beta' THEN
      UPDATE global SET beta_users = COALESCE(beta_users, 0) + 1;
    END IF;
  END IF;
  
  -- Re-enable RLS
  SET row_security = on;
END;
$$;

-- ===========================================
-- INSTRUCTIONS
-- ===========================================

-- Try OPTION 1 first (the new RLS policies above)
-- If tracking still doesn't work, try OPTION 2 (disable RLS completely)
-- If you want to keep RLS but ensure functions work, use OPTION 3 (the safe_ functions)

-- For OPTION 3, you'll need to update your JavaScript to call:
-- - safe_increment_upload_count instead of increment_upload_count
-- - safe_increment_canvas_count instead of increment_canvas_count  
-- - safe_increment_export_count instead of increment_export_count
-- - safe_update_global_stats instead of update_global_stats 